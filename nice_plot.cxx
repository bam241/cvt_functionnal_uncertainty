...
TGraphErrors* GetOneSigmaDistribution(string filename, string param){
  ...
  TH2F *hpxpy = new TH2F("hpxpy","py ps px",time_max*12,0,time_max,300,0,p_max[param]);
...
TGraph* GetOneSigmaGraph(string filename, string param, int bin){
  ...
  TFile *file1 = TFile::Open(filename.c_str()); 
  TH2F *hpxpy = new TH2F("hpxpy","py ps px",time_max*12,0,time_max,bin,0,p_max[param]);
  string command = param+"/1000.:T/12>>hpxpy";
...
void my_plot(string param){
  ...
  C1->cd(1);
  TGraphErrors* gr_err = GetOneSigmaDistribution("systematic/old/tree.root", param);
