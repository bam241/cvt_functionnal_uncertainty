...
int time_max = 200;
string full_file = "all/tree.root";
string file[6] = {"residence_time", "burnup", "cycle_length", "th_power", "separation"};
string label[6] = {"Cooling Time", "Fuel enrichment", "Cycle Time", "Thermal Power", "Separation efficiency"};
int color[6] = {4, 2, 8, 48, 38};
...
TGraphErrors* GetOneSigmaDistribution(string filename, string param) {
  ...
  tree->Draw(command.c_str(), "T/12<185.", "");
...
TGraph* add_rel_uncertainty(string name, string label, string param, TCanvas* C, int color_) {
  string filename = name + "/tree.root";
...
TGraph* GetMean(string filename, string param, int bin) {
  TFile* file1 = TFile::Open(filename.c_str());
  TH2F* hpxpy = new TH2F("hpxpy", "py ps px", time_max * 12, 0, time_max, bin,
                         0, p_max[param]);
  string command = param + "/1000.:T/12>>hpxpy";
  ...
  TGraph* gr = new TGraph(time_max * 12, x, y);
  return gr;
};
...
void my_plot(string param) {
  ...
  tree->Draw(command.c_str(),"T/12<185." , "same");
  C1->BuildLegend();
...
void nice_plot() {
  ...
  p_max["B10"] = 500000;
  err_max["B10"] = 0.1;
  legend["B10"] = "U-Nat. Consumption (cumul.) [t]";
